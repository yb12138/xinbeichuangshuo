...
func (e *GameEngine) handlePriestChoiceInput(_ string, selectionIndex int, ctxData map[string]interface{}) (bool, error) {
	...
	case "priest_divine_contract_target":
		...
		e.Log(fmt.Sprintf("%s 的 [神圣契约] 选择 %s 为目标，继续选择转移治疗值X", user.Name, target.Name))
		e.PopInterrupt()
		return true, nil
	...
	case "priest_divine_contract_x":
		...
		after := target.Heal
		if before > 4 {
			e.Log(fmt.Sprintf("%s 的 [神圣契约] 生效：移除自身%d点治疗；目标 %s 当前治疗已超过4（%d），保持不变", user.Name, xValue, target.Name, before))
		} else {
			e.Log(fmt.Sprintf("%s 的 [神圣契约] 生效：移除自身%d点治疗并转移给 %s（%d -> %d）", user.Name, xValue, target.Name, before, after))
		}
