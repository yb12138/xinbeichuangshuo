...
func newBeastSamuraiTestEngine(t *testing.T, observer model.GameObserver, enemyRole string) (*GameEngine, *model.Player, *model.Player) {
	t.Helper()
	if enemyRole == "" {
		enemyRole = "berserker"
	}
...
func requireBeastSamuraiDiscardInterrupt(t *testing.T, game *GameEngine, playerID, choiceType string) {
	t.Helper()
...
func TestBeastSamurai_InitTokens(t *testing.T) {
	game, p1, _ := newBeastSamuraiTestEngine(t, noopObserver{}, "")
...
func TestBeastSamurai_OneStrike_NextAttackIgnoresShieldAndHoly(t *testing.T) {
	...
	if promptHasOptionID(prompt, "defend") {
...
func TestBeastSamurai_BeastSoulAlert_RunsOnOtherPlayerTapped(t *testing.T) {
	...
	if reveal := findPublicDiscardReveal(obs, "p2"); reveal == nil {
