...
func setupSoulSorcererActionTurn(t *testing.T) (*engine.GameEngine, *model.Player, *model.Player) {
	t.Helper()
...
func TestSoulSorcererSoulRecall_MultiSelectSubmit(t *testing.T) {
	...
	testutils.RequireChoicePrompt(t, game, "p1", "ss_recall_pick")
...
func TestSoulSorcererSoulLink_TransferDamageBeforeResolve(t *testing.T) {
	...
	linkedHolder, _ := soulsorcererpkg.FindSoulLink(engine.NewRoleChoiceRuntime(game), p1)
	if linkedHolder == nil {
	...
	if !soulsorcererpkg.MaybeSoulLinkTransfer(engine.NewRoleChoiceRuntime(game), &game.State.PendingDamageQueue[0]) {
		t.Fatalf("expected soul link transfer prompt")
	}
	testutils.RequireChoicePrompt(t, game, "p1", "ss_link_transfer_x")
...
func TestSoulSorcererSoulLink_Replay_TransferSorcererToAlly_NoRecursiveLinkPrompt(t *testing.T) {
	...
	if soulsorcererpkg.MaybeSoulLinkTransfer(engine.NewRoleChoiceRuntime(game), &game.State.PendingDamageQueue[1]) {
...
func TestSoulSorcererSoulLink_Replay_TransferDamageThenRunsResponseChain(t *testing.T) {
	...
	testutils.RequireResponseSkillPrompt(t, game, "p3")
	testutils.ChooseResponseSkillByID(t, game, "p3", "hero_dead_duel")
	if got := p3.Gem; got != 0 {
