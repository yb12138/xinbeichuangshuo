...
func TestPrayerSwiftBlessing_AttackFollowupSurvivesPhaseEndResponseInterrupt(t *testing.T) {
	...
	if g.HasSkillResume() || g.HasPostActionEndResume() {
		t.Fatalf("expected flow resumes drained, got skill=%v post_action=%v", g.HasSkillResume(), g.HasPostActionEndResume())
	}
}
