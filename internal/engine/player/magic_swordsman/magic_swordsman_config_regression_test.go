...
func TestMagicSwordsmanShadowGather_ReleasesBeforeNextActionSelectionPrompt(t *testing.T) {
	...
	// Find the last ask-input prompt from captured events
	var lastPrompt *model.Prompt
	...
	hasMagic := false
	for _, opt := range lastPrompt.Options {
		if opt.ID == "magic" {
			hasMagic = true
			break
		}
	...
	if !hasMagic {
		t.Fatalf("expected magic option after shadow release, got %+v", lastPrompt.Options)
	}
}
