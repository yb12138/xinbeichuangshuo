...
func requireResponseSkillContains(t *testing.T, game *engine.GameEngine, playerID, skillID string) {
	t.Helper()
	testutils.RequireResponseSkillPrompt(t, game, playerID)
...
func TestHeroTaunt_NonAttackActionIsRejectedUntilSkipOrValidAttack(t *testing.T) {
	...
	testutils.MustHandleAction(t, game, model.PlayerAction{PlayerID: "p2", Type: model.CmdCannotAct})
	if testutils.GetFieldEffectCard(p2, model.EffectHeroTaunt) != nil {
...
func TestHeroDeadDuel_MagicOverflowMoraleLossFlooredToOne(t *testing.T) {
	...
	testutils.ChooseResponseSkillByID(t, game, "p1", "hero_dead_duel")
