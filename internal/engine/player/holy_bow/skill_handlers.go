...
// HolyBowAutoFillHandler implementations.
...
// Skill helper functions (prefixed with "skill" to avoid conflicts with choices.go).

func skillPairCombos(user *model.Player) []string {
	if user == nil {
		return nil
	}
...
func skillAllies(game model.IGameEngine, user *model.Player, includeSelf bool) []string {
...
func skillEnemies(game model.IGameEngine, user *model.Player) []string {
	if game == nil || user == nil {
		return nil
	}
	var ids []string
...
func skillMoraleGap(game model.IGameEngine, user *model.Player) int {
	if game == nil || user == nil {
		return 0
	}
...
func skillCanUseLightBurstModeA(user *model.Player) bool {
	if user == nil {
		return false
	}
	return user.Heal >= 1
...
func skillCanUseLightBurstModeB(game model.IGameEngine, user *model.Player, enemyIDs []string, maxX int) bool {
...
// HolyBowShardStormHandler implementations.

func (h *HolyBowShardStormHandler) CanUse(ctx *model.Context) bool {
	if ctx == nil || ctx.User == nil || ctx.Game == nil {
		return false
	}
	if len(skillPairCombos(ctx.User)) == 0 {
		return false
	}
	return len(skillEnemies(ctx.Game, ctx.User)) > 0
}
...
func (h *HolyBowShardStormHandler) Execute(ctx *model.Context) error {
	...
	combos := skillPairCombos(ctx.User)
	if len(combos) == 0 {
		return fmt.Errorf("没有可弃置的同系攻击牌组合")
	}
	enemyIDs := skillEnemies(ctx.Game, ctx.User)
	if len(enemyIDs) == 0 {
...
// HolyBowRadiantDescentHandler implementations.
...
func (h *HolyBowRadiantDescentHandler) CanUse(ctx *model.Context) bool {
	...
	return ctx.User.Heal >= 2 || Faith(ctx.User) >= 2
}
...
// HolyBowLightBurstHandler implementations.
...
func (h *HolyBowLightBurstHandler) CanUse(ctx *model.Context) bool {
	...
	allyIDs := skillAllies(ctx.Game, ctx.User, false)
	enemyIDs := skillEnemies(ctx.Game, ctx.User)
	maxX := ctx.User.Heal
	if len(ctx.User.Hand) < maxX {
		maxX = len(ctx.User.Hand)
	}
	canModeA := skillCanUseLightBurstModeA(ctx.User) && len(allyIDs) > 0
	return canModeA || skillCanUseLightBurstModeB(ctx.Game, ctx.User, enemyIDs, maxX)
}
...
func (h *HolyBowLightBurstHandler) Execute(ctx *model.Context) error {
	...
	canModeA := skillCanUseLightBurstModeA(ctx.User) && len(allyIDs) > 0
	if !canModeA && !skillCanUseLightBurstModeB(ctx.Game, ctx.User, enemyIDs, maxX) {
...
// HolyBowMeteorBulletHandler implementations.
...
func (h *HolyBowMeteorBulletHandler) CanUse(ctx *model.Context) bool {
	...
	if ctx.User.Heal <= 0 && Faith(ctx.User) <= 0 {
		return false
	}
	return len(skillAllies(ctx.Game, ctx.User, false)) > 0
}
...
// HolyBowRadiantCannonHandler implementations.
