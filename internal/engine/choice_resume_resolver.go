// gameflow: InterruptChoice 完成后的恢复点解析（单一表驱动）。
...
// ResolveChoiceInterruptResumePhase 导出给测试或其它包使用。
func ResolveChoiceInterruptResumePhase(currentPoint interface{}, ctxData map[string]interface{}) (interface{}, bool) {
	return resolveChoiceInterruptPhase(currentPoint, ctxData)
}
