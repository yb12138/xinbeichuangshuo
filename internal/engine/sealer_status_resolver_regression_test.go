...
func TestFiveElementsBind_BuffPhaseChoiceUsesSealCountCapAtTwo(t *testing.T) {
	...
	if got := countFieldEffect(p2, model.EffectFiveElementsBind); got != 1 {
...
func TestSealBreak_CanPickGlobalBasicEffectWithoutPreselectedTarget(t *testing.T) {
	...
	requireChoicePrompt(t, game, "p1", "basic_effect_pick")
	...
	if got := countFieldEffect(p3, model.EffectSealFire); got != 0 {
