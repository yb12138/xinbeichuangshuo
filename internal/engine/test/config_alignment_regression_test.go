...
func TestPendingDamage_PoisonDoesNotConsumeHolyShield(t *testing.T) {
	...
	if got := testutils.CountFieldEffect(p1, model.EffectShield); got != 1 {
...
func TestAngelCleanse_CanPickSpecificBasicEffect(t *testing.T) {
	...
	if got := testutils.CountFieldEffect(p2, model.EffectWeak); got != 1 {
		t.Fatalf("expected weakness to remain, got %d", got)
	}
	if got := testutils.CountFieldEffect(p2, model.EffectPoison); got != 0 {
