...
func buildSpiritCasterDeferredFollowupHandlers() map[string]deferredFollowupHandler {
	return map[string]deferredFollowupHandler{
		"spirit_caster_talisman": {
			label:   "SpiritCaster",
			resolve: (*GameEngine).resolveSpiritCasterTalismanFollowup,
		},
	}
}

func (e *GameEngine) beginSpiritCasterTalisman(user *model.Player, skillID string, targetIDs []string, _ []model.Card) error {
	if user == nil {
	...
	for _, targetID := range targetIDs {
		if e.State.Players[targetID] == nil {
			return fmt.Errorf("目标玩家不存在: %s", targetID)
		}
	}
